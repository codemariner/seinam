 ...
 */
function execScriptEntries(entry) {
	var execAsync = Bluebird.promisify(childProcess.exec);
	return execAsync(entry.file).then(function (stdout, stderr) {
		if (stderr) {
			console.warn('Script %s returned with error output %s', entry.script, stderr);
		}
		return stdout.join('').trim();
	}).catch(function (err) {
		console.warn('Error occurred while executing script %s', entry.file, err);
		if (entry.next) {
 ...
 */
ScriptHandler.prototype.execute = function () {
	return execScriptEntries(this.scripts[0]);
}
